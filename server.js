  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error: 'أدخل الاسم والبريد وكلمة مرور 6 أحرف على الأقل'
      });
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const results = qs.rows.map(x => {
        const selected =
          (answers[x.id] || '')
            .toUpperCase();

        const ok =
          selected === x.correct_option;
...
app.post('/api/subscribe', auth, async (req, res) => {
  ...
  try {
    const priceResult =
      await client.query(
        'SELECT subscription_price FROM site_settings WHERE id=1'
      );
...
init()
  .then(() => {
    ...
    app.listen(
      ...
      () => {
        console.log(
          `StudyMedSmart listening on ${PORT}`
        );
      }
    );
